...
struct object_data {
    ...
    char Writable_Object_Name[65];
    char Writable_Description[129];
    void *Context;
...
bool Binary_Value_Write_Property(BACNET_WRITE_PROPERTY_DATA *wp_data)
{
    ...
    switch (wp_data->object_property) {
        ...
        case PROP_OBJECT_NAME:
            ...
            if (status) {
                struct object_data *pObject =
                    Binary_Value_Object(wp_data->object_instance);
                uint16_t len = value.type.Character_String.length;
                if (pObject) {
                    memset(
                        pObject->Writable_Object_Name, 0,
                        sizeof(pObject->Writable_Object_Name));
                    if (len > 0 && len < sizeof(pObject->Writable_Object_Name)) {
                        memcpy(
                            pObject->Writable_Object_Name,
                            value.type.Character_String.value, len);
                        pObject->Writable_Object_Name[len] = 0;
                    }
        ...
        case PROP_DESCRIPTION:
            ...
            if (status) {
                ...
                if (pObject) {
                    memset(
                        pObject->Writable_Description, 0,
                        sizeof(pObject->Writable_Description));
                    if (len > 0 && len < sizeof(pObject->Writable_Description)) {
                        memcpy(
                            pObject->Writable_Description,
                            value.type.Character_String.value, len);
                        pObject->Writable_Description[len] = 0;
                    }

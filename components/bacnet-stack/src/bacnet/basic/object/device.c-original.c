...
void Device_Init(object_functions_t *object_table)
{
    ...
    // datetime_init();  // COMMENTED OUT - datetime.c not in minimal build
    if (object_table) {
    ...
    // Channel_Write_Property_Internal_Callback_Set(Device_Write_Property);  // COMMENTED OUT - channel.c not in minimal build
#endif
    /* link ReadProperty and WriteProperty to Loop object for references */
    // Loop_Read_Property_Internal_Callback_Set(Device_Read_Property);  // COMMENTED OUT - loop.c not in minimal build
    // Loop_Write_Property_Internal_Callback_Set(Device_Write_Property);  // COMMENTED OUT - loop.c not in minimal build
#if (BACNET_PROTOCOL_REVISION >= 17)
    /* link WriteProperty to Timer object for references */
    // Timer_Write_Property_Internal_Callback_Set(Device_Write_Property);  // COMMENTED OUT - timer.c not in minimal build
#endif
}

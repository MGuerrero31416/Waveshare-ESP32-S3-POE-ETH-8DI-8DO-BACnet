...
static bool COV_Initialized = false;
static unsigned cov_subscription_count(void)
{
    unsigned count = 0;
    ...
    BACNET_ERROR_CODE *error_code)
{
    ...
    debug_fprintf(stderr,
        "COV_SUBSCRIBE: request src=%p obj_type=%d obj_instance=%lu pid=%u lifetime=%lu confirmed=%u cancel=%u\n",
        (const void *)src,
        (int)cov_data->monitoredObjectIdentifier.type,
        (unsigned long)cov_data->monitoredObjectIdentifier.instance,
        (unsigned)cov_data->subscriberProcessIdentifier,
        (unsigned long)cov_data->lifetime,
        (unsigned)cov_data->issueConfirmedNotifications,
        (unsigned)cov_data->cancellationRequest);
            ...
            if ((COV_Subscriptions[index].monitoredObjectIdentifier.type ==
                ...
                if (cov_data->cancellationRequest) {
                    ...
                    cov_address_remove_unused();
                                    debug_fprintf(stderr,
                                        "COV_SUBSCRIBE: cancelled existing entry index=%d active_count=%u\n",
                                        index,
                                        cov_subscription_count());
                } else {
                    ...
                    COV_Subscriptions[index].flag.send_requested = true;
                                    dest = cov_address_get(COV_Subscriptions[index].dest_index);
                                    debug_fprintf(stderr,
                                        "COV_SUBSCRIBE: updated existing entry index=%d dest_index=%u dest=%p active_count=%u\n",
                                        index,
                                        COV_Subscriptions[index].dest_index,
                                        (const void *)dest,
                                        cov_subscription_count());
                }
    ...
    BACNET_PROPERTY_VALUE *value_list)
{
    ...
    debug_fprintf(stderr,
        "COV_SEND: start sub=%p obj_type=%d obj_instance=%lu confirmed=%u invoke_id=%u lifetime=%lu\n",
        (const void *)cov_subscription,
        (int)cov_subscription->monitoredObjectIdentifier.type,
        (unsigned long)cov_subscription->monitoredObjectIdentifier.instance,
        (unsigned)cov_subscription->flag.issueConfirmedNotifications,
        (unsigned)cov_subscription->invokeID,
        (unsigned long)cov_subscription->lifetime);
    dest = cov_address_get(cov_subscription->dest_index);
    if (!dest) {
#if PRINT_ENABLED
        debug_fprintf(stderr, "COVnotification: dest not found!\n");
        debug_fprintf(stderr,
            "COV_SEND: missing destination dest_index=%u active_count=%u\n",
            cov_subscription->dest_index,
            cov_subscription_count());
    debug_fprintf(stderr,
        "COV_SEND: destination found dest_index=%u dest=%p local_addr=%p\n",
        cov_subscription->dest_index,
        (const void *)dest,
        (const void *)&my_address);
#endif
        return status;
    }
    ...
    if (bytes_sent > 0) {
        ...
        debug_fprintf(stderr,
            "COV_SEND: transmitted bytes_sent=%d pdu_len=%d dest_index=%u invoke_id=%u\n",
            bytes_sent,
            pdu_len,
            cov_subscription->dest_index,
            cov_subscription->invokeID);
    } else {
        debug_fprintf(stderr,
            "COV_SEND: transmit failed bytes_sent=%d pdu_len=%d dest_index=%u\n",
            bytes_sent,
            pdu_len,
            cov_subscription->dest_index);
#endif
...
bool handler_cov_fsm(void)
{
    ...
    if (!COV_Initialized) {
        debug_fprintf(stderr, "COVFSM: skipped because handler_cov_init() has not completed\n");
        return false;
    }

    if (Device_Object_Functions() == NULL) {
        debug_fprintf(stderr, "COVFSM: skipped because Device_Object_Functions() is NULL\n");
        return false;
    }
    ...
    switch (cov_task_state) {
        case COV_STATE_IDLE:
                        debug_fprintf(stderr,
                            "COVFSM: idle->mark active_count=%u\n",
                            cov_subscription_count());
            index = 0;
        ...
        case COV_STATE_MARK:
            ...
            if (COV_Subscriptions[index].flag.valid) {
                ...
                if (object_type < MAX_BACNET_OBJECT_TYPE) {
                    ...
                    if (status) {
                                                debug_fprintf(stderr,
                                                    "COV_CHANGE_DETECTED: index=%d obj_type=%d obj_instance=%lu send_requested=1 active_count=%u\n",
                                                    index,
                                                    (int)object_type,
                    ...
                    debug_fprintf(stderr,
                        "COVFSM[MARK]: invalid object_type=%d for subscription=%p\n",
                        (int)object_type,
                        (const void *)&COV_Subscriptions[index]);
                }
            }
            index++;
            if (index >= MAX_COV_SUBCRIPTIONS) {
        ...
        case COV_STATE_CLEAR:
            ...
            if ((COV_Subscriptions[index].flag.valid) &&
                ...
                if (object_type < MAX_BACNET_OBJECT_TYPE) {
                    ...
                    debug_fprintf(stderr,
                        "COVFSM[CLEAR]: invalid object_type=%d for subscription=%p\n",
                        (int)object_type,
                        (const void *)&COV_Subscriptions[index]);
                }
            }
        ...
        case COV_STATE_SEND:
            ...
            if ((COV_Subscriptions[index].flag.valid) &&
                ...
                if (send) {
                                        debug_fprintf(stderr,
                                            "COV_QUEUE: index=%d obj_type=%d obj_instance=%lu dest_index=%u active_count=%u\n",
                                            index,
                                            (int)object_type,
                                            (unsigned long)object_instance,
                                            COV_Subscriptions[index].dest_index,
                    ...
                    if (object_type < MAX_BACNET_OBJECT_TYPE) {
                        ...
                        debug_fprintf(stderr,
                            "COVFSM[SEND]: invalid object_type=%d for subscription=%p\n",
                            (int)object_type,
                            (const void *)&COV_Subscriptions[index]);
                    }
                    if (status) {
    ...
    BACNET_CONFIRMED_SERVICE_DATA *service_data)
{
    ...
    debug_fprintf(stderr,
        "SUBSCRIBE_COV_RX: src=%p service_len=%u invoke_id=%u priority=%u segmented=%u\n",
        (const void *)src,
        (unsigned)service_len,
        (unsigned)service_data->invoke_id,
        (unsigned)service_data->priority,
        (unsigned)service_data->segmented_message);
    ...
    BACNET_CONFIRMED_SERVICE_DATA *service_data)
{
    ...
    debug_fprintf(stderr,
        "SUBSCRIBE_COV_PROPERTY_RX: src=%p service_len=%u invoke_id=%u priority=%u segmented=%u\n",
        (const void *)src,
        (unsigned)service_len,
        (unsigned)service_data->invoke_id,
        (unsigned)service_data->priority,

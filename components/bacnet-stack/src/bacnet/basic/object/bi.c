...
static const char *TAG = "bi_object";
    ...
    uint32_t object_instance, BACNET_BINARY_PV value)
{
    bool status = false;
    struct object_data *pObject;
    BACNET_BINARY_PV old_effective = BINARY_INACTIVE;
    bool old_storage = false;
    bool new_storage = false;
    BACNET_BINARY_PV requested_value = value;

    ESP_LOGI(TAG, "BI_SET_ENTRY instance=%lu requested=%u",
             (unsigned long)object_instance, (unsigned)requested_value);

    pObject = Binary_Input_Object(object_instance);
    if (pObject) {
        ESP_LOGI(TAG, "BI_SET_OBJECT_FOUND instance=%lu",
                 (unsigned long)object_instance);
        if (value <= MAX_BINARY_PV) {
            old_storage = pObject->Present_Value;
            old_effective = Binary_Present_Value(old_storage);
            /* de-polarize */
            if (Binary_Polarity(pObject->Polarity) != POLARITY_NORMAL) {
                if (value == BINARY_INACTIVE) {
                    value = BINARY_ACTIVE;
            ...
            new_storage = pObject->Present_Value;
            ESP_LOGI(TAG,
                     "BI_SET_APPLY instance=%lu old_storage=%u old_effective=%u new_effective=%u new_storage=%u",
                     (unsigned long)object_instance,
                     (unsigned)(old_storage ? 1 : 0),
                     (unsigned)old_effective,
                     (unsigned)value,
                     (unsigned)(new_storage ? 1 : 0));
            status = true;
        } else {
            ESP_LOGW(TAG, "BI_SET_REJECT_RANGE instance=%lu requested=%u",
                     (unsigned long)object_instance, (unsigned)requested_value);
        }
    } else {
        ESP_LOGW(TAG, "BI_SET_OBJECT_MISSING instance=%lu requested=%u",
                 (unsigned long)object_instance, (unsigned)requested_value);
    }
